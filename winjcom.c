...
static void throw_exception( JNIEnv *env, char *exc, char *func, char *msg, DWORD lastError )
{
	...
	jthrowable previous_exc = (*env)->ExceptionOccurred(env);
	
	if (previous_exc) {
        // This should never happen but if we already have an exception just clear it
        // and then continue to report the wanted exception....
        printf ("throw_exception: ERROR: clearing previous exception\n");
	...
	if ( javaclass == NULL ) {
        // this also should never happens....
        printf ("throw_exception: ERROR: cannot get exception class [%s]\n",exc);
...
(JNIEnv *env, jobject jobj, jstring port_name )
{
	...
	if ( (*env)->GetStringUTFLength(env, port_name) >= sizeof(gszPort) ){
        // the above checks that the name fits and does not overflow
        throw_exception( env, IO_EXCEPTION, "isPortPresent", "gszPort buffer too small", ERROR_SUCCESS );
        return FALSE;
    }
    else {
        int len = (*env)->GetStringLength(env, port_name);

        (*env)->GetStringUTFRegion(env, port_name, 0, len, gszPort);
 ...
 */
JNIEXPORT jlong JNICALL Java_com_engidea_win32jcom_WinjcomPort_nativeOpen
(JNIEnv *env, jobject jobj, jstring port_name)
{
	HANDLE hComm;
	char gszPort[128];
	...
	if ( (*env)->GetStringUTFLength(env, port_name) >= sizeof(gszPort) ){
        // the above checks that the name fits and does not overflow
        throw_exception( env, IO_EXCEPTION, "nativeOpen", "gszPort buffer too small", ERROR_SUCCESS );
...
(JNIEnv *env, jobject jobj, jlong jFd)
{
...
(JNIEnv *env, jobject jobj, jlong jFd, jint jwhat, jint jvalue)
{
	HANDLE hComm = (HANDLE)jFd;
	DCB dcb; 
	...
	else if ( jwhat == com_engidea_win32jcom_WinjcomPort_DCB_FLOWCONTROL ){
        switch ( jvalue ){
...
(JNIEnv *env , jobject jobj, jlong jFd, jint jwhat)
{
	HANDLE hComm = (HANDLE)jFd;
	DCB dcb; 
	...
	else if ( jwhat == com_engidea_win32jcom_WinjcomPort_DCB_BAUDRATE )
		return dcb.BaudRate;

	else if ( jwhat == com_engidea_win32jcom_WinjcomPort_DCB_DATABITS )
		return dcb.ByteSize;

	else if ( jwhat == com_engidea_win32jcom_WinjcomPort_DCB_PARITY )
		return dcb.Parity;

	else if ( jwhat == com_engidea_win32jcom_WinjcomPort_DCB_STOPBITS )
		return dcb.StopBits;
    else {
...
(JNIEnv *env, jobject job, jlong jFd, 
...
(JNIEnv *env , jobject jobj, jlong jFd, jint what, jboolean status)
{
	HANDLE hComm = (HANDLE)jFd;
	DWORD commMask;
...
(JNIEnv *env, jobject jobj, jlong jFd)
{
	...
	else if ( WaitCommEvent(hComm, &dwCommEvent, &overlapped) ) {
 ...
 jint baudrate, jint databits, jint stopbits, jint parity)
{
	...
	if ( ! GetCommState(hComm, &dcb) ) {
        ...
        else if ( ! convStopbitToWin(&dcb,stopbits)  ){
...
(JNIEnv *env, jobject jobj, jlong jfd, jint what)
{
	HANDLE hComm = (HANDLE)jfd;
	...
	*/
	...
	if ( EscapeCommFunction(hComm, what) )
        return;
	else {
...
(JNIEnv * env, jobject jobj, jlong jfd, jbyteArray jarray, jint joffset, jint jlen)
{
	HANDLE hComm = (HANDLE)jfd;
	DWORD  scritti;
	DWORD  res;
	...
	if (overlapped.hEvent == NULL) {
	...
	else if ( WriteFile(hComm, dataP+joffset, jlen, &scritti, &overlapped)) {
        ...
        else if (!GetOverlappedResult(hComm, &overlapped, &scritti, FALSE)) {
            CloseHandle(overlapped.hEvent);
...
(JNIEnv *env, jobject jobj, jlong jhandle, jbyteArray jarray, jint joffset, jint jlen)
{
	HANDLE hComm = (HANDLE)jhandle;
	DWORD  letti,res;
	jbyte *dataP;
	...
	if (overlapped.hEvent == NULL) {
	...
	else if ( ReadFile(hComm, dataP+joffset, jlen, &letti, &overlapped) ) {
        ...
        else if (! GetOverlappedResult(hComm, &overlapped, &letti, FALSE)) {
...
(JNIEnv *env , jobject jobj, jlong jFd, jint what )
{
	...
	else if ( what == com_engidea_win32jcom_WinjcomPort_COMSTAT_cbInQue )
